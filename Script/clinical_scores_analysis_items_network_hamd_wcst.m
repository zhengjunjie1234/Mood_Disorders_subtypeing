...
%% load data
[clinical_data,head] = xlsread('clinic_info.xlsx');

hc_id = xlsread('.\omics\hc.xlsx');
subtype1_id = xlsread('.\omics\subtype1.xlsx');
subtype2_id = xlsread('.\omics\subtype2.xlsx');

load('hamd.mat')
%% education
edu = xlsread('education.xlsx');
edu(isnan(edu)) = 0;
...
node_s_str_new = node_s_str;
node_s_str_new(:,1) = node_s_str(:,2);
node_s_str_new(:,2) = node_s_str(:,1);

node_s_de_new = node_s_de;
node_s_de_new(:,1) = node_s_de(:,2);
node_s_de_new(:,2) = node_s_de(:,1);



figure
title('Symptom network Nodal characters')
subplot(2,1,1)
bar(node_s_str(1:36,:))
set(gca,'xtick',[1:36],'xticklabel',node_name,'XTickLabelRotation',45)
ylabel('Nodal Strength')

subplot(2,1,2)
bar(node_s_de(1:36,:))
set(gca,'xtick',[1:36],'xticklabel',node_name,'XTickLabelRotation',45)
ylabel('Nodal Degress')
legend('Subtype 1','Subtype 2')
